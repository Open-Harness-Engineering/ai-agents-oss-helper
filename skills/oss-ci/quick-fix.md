...
If the change is too large or complex (e.g., new features, architectural changes, refactoring), stop and suggest the appropriate action from the project's `project-guidelines.md` (e.g., the Create Issue guideline (`create-issue.md`) for GitHub projects, or creating a Jira issue for Jira projects).
...
3. **Build/Test/Format**: Follow the build workflow from `_fragments/_build-workflow.md`
   ...
   - Run full reactor build (Maven projects only, MANDATORY before commit, code changes only)
   - Skip build steps for documentation-only changes or projects with no build tool
