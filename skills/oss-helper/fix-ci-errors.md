...
If you have access to agents specialized in **coding or implementation** (e.g., backend, frontend, or language-specific coding agents), you should delegate the fix implementation (step 6) to one or more of those agents. Provide them with the error analysis, affected files, and project standards. If no specialized agents are available, perform all steps directly.
...
For category B errors: create a GitHub issue (using the same approach as the Create Issue guideline (`create-issue.md`)) describing the error, its context, and the CI run link.
...
4. **Commit**: Use the CI-issue commit format from the project's `project-guidelines.md`
