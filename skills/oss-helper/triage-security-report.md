...
If you have access to agents specialized in **security analysis** (e.g., security or application-security agents), you should delegate the claim verification (step 4) to one or more of those agents. Provide them with the extracted claims, affected files, and project context. If no specialized agents are available, perform all steps directly.
