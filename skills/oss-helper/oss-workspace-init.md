...
### 5. Clone or Register Repositories
...
### 6. Detect Rules for Each Repository

For each registered repository, load its rules independently using the same priority model as the OSS Helper initialization:
...
### 7. Detect Checkout and Worktree State
...
Next:
- Workspace Status guideline (`oss-workspace-status.md`)
- Create Multi-Repo Issue guideline (`oss-create-multi-repo-issue.md`)
- Fix Multi-Repo Issue guideline (`oss-fix-multi-repo-issue.md`)
```
