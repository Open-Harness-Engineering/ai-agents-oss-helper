...
If you have access to agents specialized in **security analysis** (e.g., security or application-security agents), you should delegate the reasoning-based analysis (step 5) and finding triage (step 6) to one or more of those agents. Provide them with the threat model, scan scope, scanner output, and project context. If no specialized agents are available, perform all steps directly.
...
### 2. Establish the Threat / Security Model
...
3. **Generate a lightweight model (when none exists).** If neither of the above is present, derive a short threat model so the scan has an anchor:
   ...
   **Propose the generated model as a PR.** The threat model is a public-safe document (it describes posture, not exploits). After the scan, offer to contribute it back: write it to a sensible location (`SECURITY.md` if absent, or `docs/threat-model.md`), then hand off to the Quick Fix guideline (`quick-fix.md`) so it goes through the normal branch + PR flow with user confirmation. Do **not** fabricate a `project-security.md` CVE-handling workflow — that file cannot be inferred from source; point the user to the Add Project guideline (`add-project.md`) if they want one.
...
- **SCA / dependency findings overlap** with the Analyze Third-Party CVE guideline (`analyze-third-party-cve.md`) and the Fix GitHub Alert guideline (`fix-github-alert.md`). Note them, but in the follow-up step defer dependency CVEs to those guidelines rather than duplicating that workflow here. This guideline's primary focus is **first-party code (SAST)**.
- If running a scanner would require a network fetch, a build, or downloading rules, **ask the user first**.

### 5. Reasoning-Based Static Analysis
...
### 6. Triage & De-duplicate Findings
...
3. **File a sanitized issue** — hand off to the Create Issue guideline (`create-issue.md`) with text that frames the change as hardening/consistency/refactor and **omits** the attack scenario, payloads, severity wording, and the words "vulnerability"/"exploit"/"RCE"/"CVE". Confirm the sanitized text first.
4. **Apply a trivial safe fix** — for a low-risk, obviously-correct hardening change, hand off to the Quick Fix guideline (`quick-fix.md`) with a sanitized description.

**For a dependency / SCA finding (not first-party code):**
5. **Defer to the dependency workflow** — hand off to the Analyze Third-Party CVE guideline (`analyze-third-party-cve.md`) or, if Dependabot already raised it, the Fix GitHub Alert guideline (`fix-github-alert.md`). Do not duplicate that analysis here.

**For the generated threat model (step 3, when one was produced):**
6. **Propose it as a PR** — this is a public-safe document. Offer to write it to `SECURITY.md` / `docs/threat-model.md` and hand off to the Quick Fix guideline (`quick-fix.md`). Never include any finding detail in this document.
...
- Duplicate the dependency-CVE workflow — defer SCA/dependency findings to the Analyze Third-Party CVE guideline (`analyze-third-party-cve.md`) or the Fix GitHub Alert guideline (`fix-github-alert.md`).
