...
If you have access to agents specialized in **security analysis** (e.g., security or application-security agents), you should delegate the vulnerability analysis (steps 4–5) to one or more of those agents. Provide them with the CVE metadata, dependency coordinates, and project context. If no specialized agents are available, perform all steps directly.
