### 1. Load Workspace Repositories and Rules

Locate and load workspace metadata using the same discovery rules as the Workspace Status guideline (`oss-workspace-status.md`).

If no workspace exists, ask the user whether to run the Workspace Init guideline (`oss-workspace-init.md`) first. Do not create a multi-repo issue without an explicit repository set.

For each workspace repository:

1. Verify the path exists and is a git repository.
2. Load that repository's own rules using the OSS Helper initialization priority.
3. Read:
...
If a title was not provided, ask for a concise title.
...
### 3. Select Affected Repositories
...
### 4. Search for Duplicates Across Repositories
...
- Affected repositories.
- Suggested next step: the Fix Multi-Repo Issue guideline (`oss-fix-multi-repo-issue.md`).
...
- Create branches, commits, or PRs; fixing is handled by the Fix Multi-Repo Issue guideline (`oss-fix-multi-repo-issue.md`).
...
- The user receives the issue URL(s) and the fix handoff guideline.
