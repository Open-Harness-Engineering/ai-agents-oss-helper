...
```bash
gh issue view <ISSUE_NUMBER> --repo <GITHUB_REPO> --json number,title,body,state,labels,assignees,milestone,comments
```
...
curl -s "https://issues.apache.org/jira/rest/api/2/issue/<ISSUE_ID>" | jq '{
  ...
  comments: [.fields.comment.comments[]? | {author: .author.name, body: .body}],
  created: .fields.created,
  updated: .fields.updated
}'
```

**Rate Limiting:** For Jira, make ONE request only. Do NOT poll or make repeated requests. Fetch the assignee and comments in this same request so the ownership guard (next step) needs no extra round-trip.

### 3. Ownership Guard (STOP if the issue is already claimed)

Before doing anything that claims the issue (assigning it to the operator, transitioning it to *In Progress*, or starting the fix), apply the ownership check in `_fragments/_issue-ownership.md`.

Using the assignee and comments already retrieved in step 2:

- Confirm the issue is **Unassigned or assigned to the operator**.
- Confirm **no comment from another person** signals that they are working on it, intend to, or have asked for it (e.g. "let me work on this", "I'll open a PR", "assign to me"), and that no other contributor has linked an in-progress PR/branch.

If either check fails, **STOP**: do not assign, transition, or implement. Leave the issue's state exactly as found (revert any transition you already made), report who has claimed it (assignee or comment author), and do not continue with the steps below.
