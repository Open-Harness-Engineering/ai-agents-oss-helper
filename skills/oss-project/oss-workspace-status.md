...
If no metadata is found, stop and tell the user to run the Workspace Init guideline (`oss-workspace-init.md`) first.
...
### 2. Validate Registered Repositories
...
For each valid repository path, load its rules independently using the OSS Helper initialization priority:
...
### 4. Detect Checkout and Worktree Type
...
### 6. Detect Remote Tracking State
...
### 7. Detect Open PR for Current Branch
...
Use one PR-list request per repository. Do not poll check status from this guideline; use the PR Status guideline (`pr-status.md`) or repository-specific commands for detailed CI.
...
- Suggested next guideline, such as the Fix Multi-Repo Issue guideline (`oss-fix-multi-repo-issue.md`).
